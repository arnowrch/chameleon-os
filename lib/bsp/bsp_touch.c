...
void bsp_touch_init(esp_lcd_touch_handle_t *touch_handle, i2c_master_bus_handle_t bus_handle, uint16_t xmax, uint16_t ymax, uint16_t rotation)
{
    ...
    if (90 == rotation)
    {
        ...
        tp_cfg.flags.mirror_x = 0; // Muss möglicherweise auf 1 gesetzt werden, um die X-Achse zu spiegeln
        tp_cfg.flags.mirror_y = 1; // Muss möglicherweise auf 0 gesetzt werden, um die Y-Achse zu spiegeln
    }
